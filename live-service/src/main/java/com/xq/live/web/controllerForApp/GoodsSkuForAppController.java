...
public class GoodsSkuForAppController {
     ...
     * @return
     */
    @RequestMapping(value = "/addPlatformGoodsSkuAndSalePoint",method = RequestMethod.POST)
    public BaseResp<Long> addPlatformGoodsSkuAndSalePoint(@RequestBody @Valid GoodsSkuInVo inVo, BindingResult result,HttpServletRequest request){
        if (result.hasErrors()) {
            List<ObjectError> list = result.getAllErrors();
            return new BaseResp<Long>(ResultStatus.FAIL.getErrorCode(), list.get(0).getDefaultMessage(), null);
        }
        Long id = goodsSkuService.addPlatformGoodsSkuAndSalePoint(inVo);
        return new BaseResp<Long>(ResultStatus.SUCCESS,id);
    }
     ...
     * @return
     */
    @RequestMapping(value = "/setGoodsSkuTop", method = RequestMethod.GET)
    public BaseResp<Integer> setGoodsSkuTop(Long id){
        Integer  result=goodsSkuService.setGoodsSkuTop(id);
        return new BaseResp<Integer> (ResultStatus.SUCCESS, result);
    }
}
