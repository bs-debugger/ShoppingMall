 ...
 **/
@RestController
@RequestMapping(value = "/app/orderCoupon")
public class OrderCouponForAppController {
    @Autowired
    ...
    @RequestMapping("orderCouponExport")
    public void orderCouponExport(ModelMap modelMap, OrderCouponInVo inVo, HttpServletRequest request,
                               HttpServletResponse response) {
        if(inVo==null||inVo.getShopId()==null||inVo.getBeginTime()==null||inVo.getEndTime()==null){
            return;
        }
        TemplateExportParams params = new TemplateExportParams(
                Thread.currentThread().getContextClassLoader().getResource("").getPath() + "doc" + File.separator + "orderInfo.xlsx");
        Map<String,Object> mapColl = orderCouponService.orderCouponExport(inVo);
        Shop shopById = shopService.getShopById(inVo.getShopId());

        modelMap.put(TemplateExcelConstants.FILE_NAME, shopById.getShopName()+"财务往来对账明细");
    ...
    @RequestMapping("orderWriteCouponExport")
    public void orderWriteCouponExport(ModelMap modelMap, OrderWriteOffInVo inVo, HttpServletRequest request,
                                  HttpServletResponse response) {
        if(inVo==null||inVo.getShopId()==null||inVo.getBeginTime()==null||inVo.getEndTime()==null){
            return;
        }
        TemplateExportParams params = new TemplateExportParams(
                Thread.currentThread().getContextClassLoader().getResource("").getPath() + "doc" + File.separator + "write.xlsx");
        Map<String,Object> mapColl = orderCouponService.orderWriteCouponExport(inVo);
        Shop shopById = shopService.getShopById(inVo.getShopId());
