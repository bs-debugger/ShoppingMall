...
public class CashApplyForAppController {
    ...
    public BaseResp<?> createv1(CashApply cashApply, BindingResult result)throws ParseException{
        if (result.hasErrors()) {
            List<ObjectError> list = result.getAllErrors();
            return new BaseResp<Long>(ResultStatus.FAIL.getErrorCode(), list.get(0).getDefaultMessage(), null);
        }
        ...
        cashApply.setUserName(user.getUserName());
        //查询账户信息
        UserAccount account = accountService.findAdminAccountByUserId(cashApply.getUserId());
        if(account == null){
            return new BaseResp<Long>(ResultStatus.error_user_account_info);
        }

        //提取金额不等于账户余额，当前默认一次提取全部账户余额
        if(cashApply.getCashAmount().compareTo(BigDecimal.ZERO) <= 0|| cashApply.getCashAmount().compareTo(account.getAccountAmount()) != 0){
            return new BaseResp<Long>(ResultStatus.error_shop_cash_apply_amount);
        }

        List<CashApply> list = cashApplyService.selectListByAccountId(account.getId());
        ...
        List<UserBankInfo> bankInfos = bankService.queryListByOwnerIdAndType(cashApply.getUserId(),2);
        if(bankInfos==null||bankInfos.size()<=0){
            return new BaseResp<CashApply>(ResultStatus.error_user_BANK_UNBIND);
        }
        ...
        cashApply.setAccountId(account.getId());
        cashApply.setApplyType(CashApply.CASH_APPLY_TYPE_SHOP);
        cashApply.setAccountCardholderName(bankInfos.get(0).getName());
        cashApply.setBankCardName(bankInfos.get(0).getBankName());
        cashApply.setAccountName(bankInfos.get(0).getCardNo());
        Long res = cashApplyService.create(cashApply);
        return new BaseResp<Long>(ResultStatus.SUCCESS, res);
    }
    ...
    public BaseResp<?> list(CashApplyInVo inVo){
        ...
        inVo.setApplyType(CashApply.CASH_APPLY_TYPE_SHOP);
        UserAccount adminAccount = accountService.findAdminAccountByUserId(inVo.getUserId());
        if(adminAccount == null){
            return new BaseResp<Long>(ResultStatus.error_user_account_info);
        }
        inVo.setAccountId(adminAccount.getId());
        inVo.setShopId(adminAccount.getShopId());
        inVo.setUserId(null);
    ...
    public BaseResp<CashApply> selectByShopId(Long shopId)throws ParseException {
        ...
        if(cashApply.getApplyStatus()==CashApply.CASH_APPLY_STATUS_WAIT&&cashApply.getApplyType()==CashApply.CASH_APPLY_TYPE_SHOP){
            return  new BaseResp<CashApply>(ResultStatus.error_shop_have_apply);
        }
