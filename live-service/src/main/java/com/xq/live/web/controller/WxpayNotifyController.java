...
public class WxpayNotifyController {
    ...
    private Logger logger = Logger.getLogger(WxpayNotifyController.class);
    ...
    public WxpayNotifyController() throws Exception {
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/wxNotifyForCouponSendAmountV1")
    public void wxNotifyForCouponSendAmountV1(HttpServletRequest request, HttpServletResponse response) throws Exception {
        //读取参数
        BufferedReader br = new BufferedReader(new InputStreamReader((ServletInputStream) request.getInputStream()));
        String line = null;
        StringBuilder sb = new StringBuilder();
        ...
        if (wxpay.isPayResultNotifySignatureValid(notifyMap)) {
            ...
            if ("SUCCESS".equals(notifyMap.get("result_code"))) {
                ...
                WeixinInVo attachInvo = JSON.parseObject(attach, WeixinInVo.class);//将附带参数读取出来
                OrderCouponPostageInVo postageInVo = new OrderCouponPostageInVo();
                postageInVo.setId(attachInvo.getOrderCouponPostageId());
                OrderCouponPostage orderCouponPostage=orderCouponPostageService.selectInfo(postageInVo);
                OrderCoupon couponOut = orderCouponService.getByCouponCode(orderCouponPostage.getOrderCouponCode());//couponCode
                if (!PaymentConfig.MCH_ID.equals(mch_id) || couponOut == null) {
                    ...
                    if (OrderCoupon.ORDER_COUPON_IS_USED_NO == couponOut.getIsUsed()) {//票券的状态判断
                        ...
                        //添加日志
                        orderCouponService.paidCouponSendAmount(inVo);

                        //修改票券状态
                        User user = userService.getUserById(attachInvo.getUserId());
                        ...
                        couponInVo.setId(orderCouponPostage.getOrderCouponId());
                        couponInVo.setSendTime(orderCouponPostage.getSendTime().toString());
                        couponInVo.setRemark(orderCouponPostage.getRemark());
                        couponInVo.setSendAmount(orderCouponPostage.getSendAmount());
                        couponInVo.setCouponAddressId(orderCouponPostage.getCouponAddressId());
                        couponInVo.setShopId(orderCouponPostage.getShopId());

                        //设置商家名字
                        if(new Long(0).toString().equals(orderCouponPostage.getShopId().toString())){
                            couponInVo.setShopName("享七自营");
                        }else{
                            Shop shopById = shopService.getShopById(orderCouponPostage.getShopId());
                            couponInVo.setShopName(shopById.getShopName());
                        }

                        //判断票券是否存在或票券是否被使用
                        OrderCouponOut cp = orderCouponService.selectById(orderCouponPostage.getOrderCouponId());
                        if(cp==null||cp.getIsUsed()== OrderCoupon.ORDER_COUPON_IS_USED_YES){
                            logger.info(ResultStatus.error_coupon_is_used);
                        }
                        //退款申请中和已退款的票卷不能使用
                        if(cp.getStatus()!=null&&(cp.getStatus()==OrderCoupon.STATUS_REFUND||cp.getStatus()==OrderCoupon.STATUS_REFUND_APPLICATION)){
                            logger.info(ResultStatus.error_coupon_refund);
                        }
                        //判断订单是否存在
                        OrderInfo orderInfo = orderInfoService.get(cp.getOrderId());
                        if(orderInfo==null){
                            logger.info(ResultStatus.error_so_not_exist);
                        }
                        //修改票券
                        orderCouponService.useCoupon(couponInVo);
                        //修改运费订单状态
                        orderCouponPostageService.updateOrderStatus(postageInVo);
                        //增加抽奖次数
     ...
     * @param orderOut
     */
    public void pushOrderInfo(OrderInfo orderOut,String totalFee){
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        java.util.Date date=new java.util.Date();
        String str=sdf.format(date);
