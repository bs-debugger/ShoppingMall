...
/*    @RequestMapping(value = "/delete/{id}", method = RequestMethod.DELETE)
    ...
    public BaseResp<Integer> update(Shop shop) {
        ...
        if (shop!=null&&shop.getShopName()!=null){
            List<Shop> list = shopService.DuplicateCheckingByShopName(shop);
            if (list!=null&&list.size()>0){
                return new BaseResp<Integer>(ResultStatus.error_shop_duplicate_checking_by_shopname);
            }
        }
     ...
     * @param inVo
     * @return
     * app/shop/subbranchListV1?id=35&beginTime=2018-06-01&endTime=2018-08-01
     */
    @RequestMapping(value = "/subbranchListV1", method = RequestMethod.GET)
    public BaseResp<List<Map<String, String>>> subbranchListV1(ShopInVo inVo) {
        List<Map<String, String>> subOut = shopService.subbranchListV1(inVo);
        if (subOut==null||subOut.size()<1){
            return new BaseResp<List<Map<String, String>>>(ResultStatus.getError_allocation_selectList);
        }
        return new BaseResp<List<Map<String, String>>>(ResultStatus.SUCCESS, subOut);
    }
